...
type HeroDualCtaProps = {
  ...
  /**
   * Provide plain text. Newlines are supported (rendered via whitespace-pre-line).
   * Do NOT include HTML tags here.
   */
  subtitle?: string;
  ...
  scrollCueLabel?: string; // e.g. "Scroll for the Truth ↓"
};
...
Built for serious landowners who want consistent opportunities at mature bucks — not hope, hype, or habitat theater.`,
  ...
  scrollCueLabel = "Scroll for the Truth ↓",
}: HeroDualCtaProps) {
  // Ensure hrefs are always defined and safe
  const safePrimaryHref = primaryHref || "/property-audit";
  const safeSecondaryHref = secondaryHref || "/properties";
        ...
        <div className="wls-hero-fadeup max-w-3xl">
          ...
          <div className="mt-6 text-sm text-white/75">
            <span className="font-semibold text-white/90">
              Application required • Limited monthly availability • Not for casual hunters
            </span>
          </div>

          {/* Scroll cue (safe anchor link) */}
          {showScrollCue && (
            <div className="mt-10">
              <a
                href={scrollCueHref}
                className="inline-flex items-center gap-2 text-sm font-semibold text-white/75 underline decoration-white/20 underline-offset-4 transition hover:text-white hover:decoration-white/40"
              >
                {scrollCueLabel}
              </a>
            </div>
